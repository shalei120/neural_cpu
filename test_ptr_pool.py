...
class PTRModel(object):
    def __init__(self, I, O, N, W, K, G, sess, min_length = 1, max_length = 12,
        ...
        self.controller = PTRCell([(N,W), (N,W)], [1, 1], [0,  1], (1, W), controller_layer_size=controller_layer_size, addr_mode=0)
        self.sess = sess
        self.scope = scope
        ...
        with tf.variable_scope(self.scope):
            ...
            if not forward_only:
                for seq_length in range(self.min_length, self.max_length ):
                    ...
                    cn = self.c * tf.pow(tf.to_float(seq_length), self.a) + self.b
    ...
    def get_stop_pos(self, seq_length):
        ...
        max_pos = tf.clip_by_value(self.c * tf.pow(tf.to_float(seq_length), self.a) + self.b, 0, self.MAX_STEP - 1)
        stop_pos = D(self.MAX_STEP, max_pos, 1, self.beta)
