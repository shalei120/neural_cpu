...
class PTRModel(object):
    def __init__(self, I, O, N, W, K, G, sess, min_length = 1, max_length = 12,
        ...
        self.controller = PTRCell([(N,W), (N,W), (N,W)], [1, 1, 1], [0, 0, 1], (1, W), controller_layer_size=controller_layer_size, addr_mode=0)
        self.sess = sess
        self.scope = scope
    ...
    def build_model(self, forward_only):
        ...
        with tf.variable_scope(self.scope):
            ...
            self.exp = weight('exp', [1,1])
            self.exp_weight = weight('exp_weight', [1, 1])
            ...
            if not forward_only:
                for seq_length in range(self.min_length, self.max_length + 1):
                    ...
                    cn = tf.pow(tf.to_float(seq_length), self.exp) + self.exp_bias
    ...
    def get_stop_pos(self, seq_length):
        ...
        max_pos = tf.clip_by_value(tf.pow(tf.to_float(seq_length), self.exp) + self.exp_bias, 0, self.MAX_STEP - 1)
        stop_pos = D(self.MAX_STEP, max_pos, 1, self.beta)
        return tf.argmax(stop_pos[0]), max_pos, self.exp, self.exp_weight, self.exp_bias, self.beta
...
def train(sess):
    ...
    # model.load(FLAGS.checkpoint_dir, 'math' + sys.argv[1] + '_' + sys.argv[2])
    ...
    for idx in range(FLAGS.epochs):
        ...
        if idx % 100 == 0 or idx == FLAGS.epochs-1:
            model.save(FLAGS.checkpoint_dir, 'math' + sys.argv[1] + '_' + sys.argv[2], step)
    ...
    with tf.Session(config=config) as sess:
        if FLAGS.is_train:
            ...
            model = PTRModel(FLAGS.input_dim, FLAGS.output_dim, FLAGS.N, FLAGS.W, FLAGS.K, FLAGS.G,
                      sess, min_length=FLAGS.min_length, max_length=FLAGS.max_length, MAX_STEP=FLAGS.MAX_STEP,
                      lr=FLAGS.learning_rate, epsilon=FLAGS.epsilon, controller_layer_size=FLAGS.controller_layer_size)
            model.load(FLAGS.checkpoint_dir, 'math' + '_' + sys.argv[2])
