...
class PTRModel(object):
    ...
    def build_model(self, forward_only):
        ...
        with tf.variable_scope(self.scope):
            ...
            self.exp_weight = tf.constant([[1.0]], dtype=tf.float32)#weight('exp_weight', [1, 1])
            self.exp_bias = weight('exp_bias', [1,1], init = 'constant', value = 0.0)
            ...
            if not forward_only:
                for seq_length in range(self.min_length, self.max_length + 1):
                    ...
                    max_pos = tf.clip_by_value(self.exp_weight * tf.pow(tf.to_float(seq_length), self.exp) + self.exp_bias, 0, self.MAX_STEP-1)
...
def test(model, seq_length, sess, print_=True):
    ...
    f = open('test_case_sort_%d' % seq_length, 'wb')
