...
from ptr_cell import PTRCell, mod
...
class PTRModel(object):
    def __init__(self, I, O, N, W, K, G, sess, min_length = 1, max_length = 10,
        ...
        self.controller = PTRCell([(N,W), (N,W), (N*N,W)], [1, 1,0], [0, 0, 1], (1, W), controller_layer_size=controller_layer_size, addr_mode=0)
        self.sess = sess
        self.scope = scope
    ...
    def build_model(self, forward_only):
        ...
        with tf.variable_scope(self.scope):
            ...
            for seq_length in range(1, self.max_length + 1):
                ...
                for j in range(0, seq_length*seq_length+1):
                ...
                self.outputs[seq_length] = tf.unstack(state['M'][-1][0:seq_length*seq_length])
            ...
            if not forward_only:
                for seq_length in range(self.min_length, self.max_length + 1):
                    ...
                    loss = sequence_loss(logits=self.outputs[seq_length],
                                         targets=self.true_outputs[0:seq_length*seq_length],
...
def generate_sequence(length, bits):
    ...
    for idx in range(length):
        seq[idx, 0:bits] = np.round(np.random.rand(bits) * 100) %3 +1
    return list(seq)
...
def test(model, seq_length, sess, print_=True):
    ...
    feed_dict.update(
        {input_:vec for vec, input_ in zip(onehot(seq2, seq_length), model.inputs_2)} 
    )
    feed_dict.update(
        {true_output:vec for vec, true_output in zip(onehot(seq, seq_length*seq_length), model.true_outputs)}
    )
    ...
    outputs = list(np.reshape(np.argmax(np.asarray(outputs), 1), [seq_length*seq_length, 1]))
    f = open('test_case_outpb_%d' % seq_length, 'wb')
    ...
    if print_:
        ...
        pprint(np.hstack([np.asarray(seq), np.zeros((seq_length*seq_length, 1)), np.asarray(outputs)]))
        print(" Loss : %f" % loss)
...
def train(sess):
    ...
    model = PTRModel(FLAGS.input_dim, FLAGS.output_dim, FLAGS.N, FLAGS.W, FLAGS.K, FLAGS.G,
              sess, min_length=FLAGS.min_length, max_length=10,
    ...
    for idx in range(FLAGS.epochs):
        ...
        if idx % 100 == 0 or idx == FLAGS.epochs-1:
            model.save(FLAGS.checkpoint_dir, 'mul', step)
