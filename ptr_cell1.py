...
class PTRCell(object):
    ...
    def init_state(self, dummy_value=0.0):
        ...
        state = {
            ...
            'R': R,
            'val': val,
            'ptr': tf.zeros([1, self.ptr_size]), #Linear(dummy, self.ptr_size, name='ptr_init'),
    ...
    def update_memory(self, state, X_list, limit=0.8):
         ...
         if self.addr_mode == 0:
             ...
             similarity = C(state['M'][0], X_list[0], 1)
             ...
             addr = tf.cond(values[0][0] > limit, lambda: C(state['M'][0], X_list[0], 100), lambda: D(self.M_shapes[0][0], tf.constant([[seq]], dtype=tf.float32), 1, 100))
             for i in range(len(self.M_shapes)):
    ...
    def __call__(self, state):
        #tf.constant([[state['seq_length']]], dtype=tf.float32)
            ...
            if self.addr_mode == 0:
                ...
                for i in range(len(self.M_shapes)):
                    if self.read_ptrs[i] != 0:
                        a_r = D(self.M_shapes[i][0], ptr[:, read_ptr_start:read_ptr_start+self.read_ptrs[i]], self.read_ptrs[i], beta_r) # k * length
                    ...
                    if self.write_ptrs[i] != 0:
                        a_w = D(self.M_shapes[i][0], state['ptr'][:, write_ptr_start:write_ptr_start+self.write_ptrs[i]], self.write_ptrs[i], beta_w) # k * length
                ...
                for i in range(len(self.M_shapes)):
                    ...
                    if self.write_ptrs[i] != 0:
                        c_w = tf.reshape(Y[:, write_start:write_start+self.write_sizes[i]], [self.write_ptrs[i], self.M_shapes[i][1]])
                        M.append(state['M'][i] * tf.transpose(tf.reduce_sum(1.0 - a, 0, keep_dims=True)) + tf.matmul(tf.transpose(a), c_w))
                        write_start += self.write_sizes[i]
                    else:
                        M.append(state['M'][i])
                ptr = state['ptr']
