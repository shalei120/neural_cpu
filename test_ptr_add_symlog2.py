...
class PTRModel(object):
    ...
    def build_model(self, forward_only):
        ...
        with tf.variable_scope(self.scope):
            ...
            for seq_length in range(1, self.max_length + 2):
            ...
            for seq_length in range(1, self.max_length + 1):
                ...
                for j in range(self.MAX_STEP):
                    ...
                    candidate_outputs.append(tf.unstack(state['M'][-1][0:(seq_length+1)]))
                ...
                for seq_length in range(self.min_length, self.max_length ):
                    ...
                    for index in range(self.MAX_STEP):
                        loss = sequence_loss(logits=self.outputs[seq_length][index],
                                             targets=self.true_outputs[0:seq_length+1],
                                             weights=[1] * (seq_length+1),
                    ...
                    cn = tf.pow(tf.to_float(seq_length), self.a) + tf.pow(tf.to_float(seq_length), self.c) * tf.pow(tf.log(tf.to_float(seq_length)), self.d)+ self.b
    ...
    def get_stop_pos(self, seq_length):
        ...
        max_pos = tf.clip_by_value(tf.pow(tf.to_float(seq_length), self.a) + tf.pow(tf.to_float(seq_length), self.c) * tf.pow(tf.log(tf.to_float(seq_length)), self.d)+ self.b, 0, self.MAX_STEP - 1)
        stop_pos = D(self.MAX_STEP, max_pos, 1, self.beta)
        return tf.argmax(stop_pos[0]), max_pos, self.a, self.c, self.d,self.b, self.beta
...
def test(model, seq_length, sess, print_=True):
    ...
    feed_dict.update({true_output:vec for vec, true_output in zip(onehot(seq, seq_length+1), model.true_outputs)})
    ...
    stop_pos, max_pos, a, c, d, b, beta = sess.run(model.get_stop_pos(seq_length), feed_dict=feed_dict)

    print('test pos:', stop_pos, max_pos, a, c, d, b, beta)
    ...
    outputs = list(np.reshape(np.argmax(np.asarray(outputs), 1), [seq_length+1, 1]))
    ...
    if print_:
        ...
        pprint(np.hstack([np.asarray(seq), np.zeros((seq_length+1, 1)), np.asarray(outputs)]))
        print(" Loss : %f" % loss)
...
def train(sess):
    ...
    # model.load(FLAGS.checkpoint_dir, 'mathlog2_' + sys.argv[1] + '_' + sys.argv[2])
    ...
    for idx in range(FLAGS.epochs):
        ...
        if idx % 100 == 0 or idx == FLAGS.epochs-1:
            model.save(FLAGS.checkpoint_dir, 'mathlog2_' + sys.argv[1] + '_' + sys.argv[2], step)
...
def Gen_test(seq_length):
    ...
    return res
testset = Gen_test(int(sys.argv[2]))
