...
from ptr_cell_1M import PTRCell, D
...
class PTRModel(object):
    def __init__(self, I, O, N, W, K, G, sess, min_length=1, max_length=10,
        ...
        self.controller = PTRCell([(N, W)], [1], [1], (1, W),
...
def test(model, seq_length, sess, print_=True):
    seq1 = generate_sequence(seq_length/2, 1)
    seq2 = generate_sequence(seq_length/2, 1)
    seq_in = []
    for i in range(int(seq_length/2)):
        seq_in.append(seq1[i])
        seq_in.append(seq2[i])

    seq = op(seq1, seq2, seq_length)

    # print(seq1,seq2,seq_in)

    feed_dict = {input_: vec for vec, input_ in zip(onehot(seq_in, seq_length), model.inputs_1)}
