...
class PTRModel(object):
    ...
    def build_model(self, forward_only):
        ...
        with tf.variable_scope(self.scope):
            self.length2stepdist = tf.get_variable('embedding', [self.max_length+1, self.max_length+1])   # for each length we have a vector with max_length representing the step dist
            ...
            for seq_length in range(1, self.max_length + 1):
                ...
                self.step_dist1 = tf.nn.embedding_lookup(self.length2stepdist, [seq_length])
                ...
                self.outputs[seq_length] = candidate_outputs  #tf.unstack(state['M'][-1][0:seq_length])
                ...
                if earlystop:
                    ...
                    for j in range(0,seq_length+1)[::-1]:
                        ...
                        cur_state = self.collect_states[seq_length][self.collect_base+j]
                        self.outputs[seq_length] = tf.cond(self.stops[j][1]>self.stops[j][0], lambda: tf.unstack(cur_state['M'][-1][0:seq_length]), lambda: self.outputs[seq_length])
            ...
            if not forward_only:
                for seq_length in range(self.min_length, self.max_length + 1):
                    ...
                    loss1 = tf.reduce_sum(tf.expand_dims(all_losses, 0) * self.step_dist, axis = 1) + 0.002 * tf.to_float(maxpos[1])
...
def test(model, seq_length, sess, print_=True):
    ...
    posdist, stop_pos, step1 = sess.run(model.get_stop_pos(seq_length), feed_dict=feed_dict)

    print('test pos:', stop_pos, posdist,step1)
